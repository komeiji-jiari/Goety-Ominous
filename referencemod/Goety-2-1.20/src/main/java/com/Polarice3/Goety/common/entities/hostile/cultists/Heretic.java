...
public class Heretic extends Cultist implements IHeretic {
    private static final EntityDataAccessor<Boolean> CHANTING = SynchedEntityData.defineId(Heretic.class,
            EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> CASTING = SynchedEntityData.defineId(Heretic.class,
            EntityDataSerializers.BOOLEAN);
        ...
        if (!this.level.isClientSide) {
            ...
            if (this.getMonolith() == null) {
                if (this.getLeader() instanceof ObsidianMonolith obsidianMonolith) {
                    ...
                    for (ObsidianMonolith monolith1 : this.level.getEntitiesOfClass(ObsidianMonolith.class,
    ...
    public void handleEntityEvent(byte pId) {
        if (pId == 4) {
            ...
            for (int i = 0; i < this.random.nextInt(35) + 10; ++i) {
                ...
                this.level.addParticle(ModParticleTypes.RISING_ENCHANT.get(),
                        this.getX() + this.random.nextGaussian() * (double) 0.13F,
                        this.getBoundingBox().maxY + 0.5D + this.random.nextGaussian() * (double) 0.13F,
                        this.getZ() + this.random.nextGaussian() * (double) 0.13F, colorUtil.red(), colorUtil.green(),
                        colorUtil.blue());
            }
        } else {
    ...
    public static class ChantAtTargetGoal extends Goal {
        ...
        public boolean canUse() {
            ...
            return target != null && target.isAlive()
                    && this.heretic.distanceTo(target) <= this.heretic.getAttributeValue(Attributes.FOLLOW_RANGE)
                    && this.heretic.hasLineOfSight(target);
        }
    ...
    public static class CastingGoal extends Goal {
        ...
        public boolean canUse() {
            LivingEntity target = this.heretic.getTarget();
            return target != null && target.isAlive() && !this.heretic.getConvokePos().isEmpty()
                    && this.heretic.getCastCoolDown() <= 0;
        }
            ...
            if (!this.heretic.getConvokePos().isEmpty()) {
                for (Vec3 vec31 : this.heretic.getConvokePos()) {
                    if (this.heretic.getNavigation().isStableDestination(BlockPos.containing(vec31))) {
                        Optional<Vec3> interceptPos = this.heretic.getBoundingBox().clip(vec31,
                                vec31.add(1.0D, 1.0D, 1.0D));
                        if (this.heretic.getBoundingBox()
                                .inflate(this.heretic.getAttributeValue(Attributes.FOLLOW_RANGE)).contains(vec31)) {
                            if (0.0D <= hitDist) {
                ...
                try {
                    if (this.targetPos != null) {
                        try {
                            for (Heretic heretic1 : this.heretic.level.getEntitiesOfClass(Heretic.class,
                                    this.heretic.getBoundingBox()
                                            .inflate(this.heretic.getAttributeValue(Attributes.FOLLOW_RANGE)))) {
            ...
            if (this.targetPos != null) {
                ...
                if (this.heretic.distanceToSqr(this.targetPos) > Mth.square(4.0D)) {
                    ...
                    if (this.castingTime == TOTAL_CAST_TIME) {
                        ...
                        if (this.heretic.level instanceof ServerLevel serverLevel) {
                            summon.finalizeSpawn(serverLevel,
                                    this.heretic.level.getCurrentDifficultyAt(BlockPos.containing(this.targetPos)),
    ...
    public static class PrayToMonolithGoal extends Goal {
        ...
        @Override
        public boolean canUse() {
            return this.heretic.getMonolith() != null && this.heretic.getMonolith().isAlive()
                    && this.heretic.getHealth() == this.heretic.getMaxHealth();
        }
        ...
        public void tick() {
            if (this.heretic.getMonolith() != null) {
                if (this.heretic.distanceTo(this.heretic.getMonolith()) <= 8.0D
                        && this.heretic.hasLineOfSight(this.heretic.getMonolith())) {

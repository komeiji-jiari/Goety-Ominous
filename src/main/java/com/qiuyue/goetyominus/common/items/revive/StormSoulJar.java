...
import com.qiuyue.goetyominus.common.entities.ally.mobs.StormNecromancerServant;
...
public class StormSoulJar extends SoulJar {
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (getSummon(stack, level) != null){
            ...
            entity = new StormNecromancerServant(ModEntityTypes.STORM_NECROMANCER_SERVANT.get(), level);
            IOwned owned = (IOwned) entity;
            owned.setTrueOwner(player);
        }
        if (entity instanceof StormNecromancerServant necromancer) {
            if (target instanceof RattledServant) {
                if (necromancer.getTrueOwner() == player) {
                    if (RitualRequirements.canSummon(level, player, ModEntityTypes.STORM_NECROMANCER_SERVANT.get())) {
    ...
    public static boolean isStorm(ItemStack stack) {
        CompoundTag compoundtag = stack.getTag();
        return stack.getItem() instanceof StormSoulJar && compoundtag != null && compoundtag.contains(TAG_STORM);
    }

    public static void setStorm(ItemStack stack){
        CompoundTag compoundTag = stack.getOrCreateTag();
        compoundTag.putBoolean(TAG_STORM, true);
    }
}

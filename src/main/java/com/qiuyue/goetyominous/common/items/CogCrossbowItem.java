...
public class CogCrossbowItem extends CrossbowItem implements Vanishable {
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity living, int timeLeft) {
        ...
        if (f >= 1.0F && !isCharged(stack) && tryLoadProjectiles(living, stack)) {
            ...
            int maxShots = 5 + msBonusShots;
            List<ItemStack> loaded = getChargedProjectiles(stack);
            int loadedCount = loaded.isEmpty() ? 0 : loaded.get(0).getCount();
            setChargedShots(stack, Math.min(loadedCount, maxShots));
    ...
    protected static void shootProjectile(Level level, LivingEntity shooter, InteractionHand hand,
        ...
        if (isFirework) {
            ...
            arrow.getPersistentData().putBoolean("goetyominous:no_invul", true);
            projectile = arrow;
        }

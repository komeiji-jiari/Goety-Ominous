...
public class BlackBeastArmorSoulRepairHandler {

    @SubscribeEvent
    public static void onLivingTick(LivingEvent.LivingTickEvent event) {
        LivingEntity entity = event.getEntity();
        if (entity.level().isClientSide) return;
        if (!(entity instanceof BlackBeast)) return;
        ...
        if (!(armor.getItem() instanceof DarkBlackBeastArmorItem)) return;
        ...
        if (SEHelper.getSoulsAmount(owner, cost) && entity.tickCount % com.qiuyue.goetyominous.config.WeaponConfig.BlackBeastArmorSoulRepairInterval.get() == 0) {

...
/**
 * S2C:把核能苦力怕仆从爆炸的"无辐照 zone"同步给爆炸附近的客户端。
 *
 ...
 * 服务端已由事件拦截,但客户端实体仍会被客户端爆炸 tick 直接施加效果/速度,且服务端不会
 * 同步移除 → 绿色视觉、血条着色、效果时长显示永久残留,直到重进存档才恢复;冲击波击退也会
 * 把本地玩家(位置由客户端主导)和友军掀飞。
 *
 * 此包把 zone(位置+半径+游戏刻过期+ownerIds)同步到客户端,让客户端的 onEffectApplicable
 * 同样拒绝辐照、onLivingTick 中和冲击波击退。zone 与服务端 protectOwnerAndServants 登记的
 * 完全一致,客户端按 level.getGameTime() 过期(与服务端同步)。
 */

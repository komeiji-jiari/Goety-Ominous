...
public class Disciple extends AbstractGOCultist implements ICultist {
    ...
    static {
        CASTING = SynchedEntityData.defineId(Disciple.class, EntityDataSerializers.BOOLEAN);
        CURRENT_SPELL = SynchedEntityData.defineId(Disciple.class, EntityDataSerializers.INT);
    }
    ...
    class CastingSpellGoal extends Goal {
        ...
        public boolean canUse() {
            return Disciple.this.isCasting();
        }

        public void start() {
            super.start();
            Disciple.this.getNavigation().stop();
        }

        public void stop() {
            super.stop();
            Disciple.this.setCasting(false);
            Disciple.this.level().broadcastEntityEvent(Disciple.this, (byte) 7);
            Disciple.this.setCurrentSpell(0);
            Disciple.this.setCurrentAnimationState(IDLE);
            Disciple.this.coolDown = 20;
        }

        public void tick() {
            LivingEntity target = Disciple.this.getTarget();
            if (target != null) {
                MobUtil.instaLook(Disciple.this, target);
            }
            Disciple.this.getNavigation().stop();
            Disciple.this.getMoveControl().strafe(0.0F, 0.0F);
        }
    ...
    class SpellGoal extends Goal {
        ...
        public boolean canUse() {
            ...
            if (target != null && target.isAlive()) {
                return !Disciple.this.isCasting()
                        && Disciple.this.hasLineOfSight(target)
                        && Disciple.this.coolDown <= 0
                        && Disciple.this.spellCooldown <= 0;
            }
            return false;
        }

        public boolean canContinueToUse() {
            return this.castingTime > 0 && Disciple.this.isCasting();
        }
        ...
        private void selectSpell() {
            ...
            for (DiscipleSpell spell : DiscipleSpell.values()) {
                if (spell == DiscipleSpell.ZOMBIE_VILLAGER && Disciple.this.zombieVillagerCount >= 8) {
                    continue;
                }
                if (spell == DiscipleSpell.METEOR && Disciple.this.meteorCooldown > 0) {
                    continue;
                }
                spells.add(spell);
            }
            ...
            if (!spells.isEmpty()) {
                DiscipleSpell chosen = spells.get(Disciple.this.getRandom().nextInt(spells.size()));
                ...
                if (spellEnum.spell instanceof IChargingSpell chargingSpell) {
                    this.castingTime = chargingSpell.shotsNumber(Disciple.this, ItemStack.EMPTY);
                    this.chargeTicks = 10;
                } else {
                ...
                switch (chosen) {
                    ...
                    case DAMNED:
                        Disciple.this.setCurrentAnimationState(SUMMON);
                        break;
                    default:
                        Disciple.this.setCurrentAnimationState(ATTACK);
                        break;
                }

                Disciple.this.setCurrentSpell(this.selectedSpell);
                Disciple.this.setCasting(true);
                Disciple.this.level().broadcastEntityEvent(Disciple.this, (byte) 6);
                Disciple.this.playSound(ModSounds.APOSTLE_PREPARE_SPELL.get(), 1.0F, 1.0F);
            } else {
                Disciple.this.coolDown = 20;
            }
        }
        ...
        public void tick() {
            ...
            if (spellEnum.spell instanceof IChargingSpell) {
                if (!spellEnum.spell.conditionsMet(Disciple.this.level(), Disciple.this)) {
                    this.cancelSpell();
                    return;
                }
                --this.chargeTicks;
                if (this.chargeTicks <= 0) {
                    ...
                    SpellStat spellStat = WandUtil.getStats(Disciple.this, spell1);
                    spell1.mobSpellResult(Disciple.this, ItemStack.EMPTY, spellStat);
                    this.chargeTicks = 10;
                }
                Disciple.this.level().broadcastEntityEvent(Disciple.this, (byte) 4);
            } else {
                Disciple.this.level().broadcastEntityEvent(Disciple.this, (byte) 5);
            }
            spellEnum.spell.useParticle(Disciple.this.level(), Disciple.this, ItemStack.EMPTY);
        ...
        private void castSpell() {
            ...
            if (!(spellEnum.spell instanceof IChargingSpell)) {
                spellEnum.spell.mobSpellResult(Disciple.this, ItemStack.EMPTY, WandUtil.getStats(Disciple.this, spellEnum.spell));
            }

            if (this.selectedSpell == DiscipleSpell.METEOR.id) {
                Disciple.this.meteorCooldown = 400;
            }

            Disciple.this.spellCooldown = 40 + Disciple.this.getRandom().nextInt(40);
        }
    ...
    public static class FireBlastDefenseGoal extends Goal {
        private final Disciple disciple;
        private int chargeTime;

        public FireBlastDefenseGoal(Disciple disciple) {
    ...
    static class ZombieVillagerServantSummonSpell extends SummonSpell {
        ...
        public void mobSpellResult(LivingEntity caster, ItemStack staff, SpellStat stats) {
            ...
            Disciple disciple = (Disciple) caster;

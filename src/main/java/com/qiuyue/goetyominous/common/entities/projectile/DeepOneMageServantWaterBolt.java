...
public class DeepOneMageServantWaterBolt extends WaterHurtingProjectile {
    ...
    public void tick() {
        ...
        if (!this.level().isClientSide) {
            ...
            if (target != null && (this.tickCount > 3 || (double) this.seekAmount > 0.3)
                    && this.dieIn == -1 && this.distanceTo(target) > 1.5F
                    && (this.tickCount < 20 || ((double) this.seekAmount > 0.3 && this.tickCount < 40))) {
                Vec3 vec3 = target.position().add(0.0D, (double) (target.getBbHeight() * 0.85F), 0.0D)
                        .subtract(this.position()).normalize();
                float f = 1.0F - (this.seekAmount - 0.3F) * 0.3F;
                this.setDeltaMovement(this.getDeltaMovement().scale((double) f).add(vec3.scale((double) this.seekAmount)));
            }
        } else {
        ...
        if (this.level().getBlockStates(this.getBoundingBox()).noneMatch(BlockState::isAir) && !this.isInWaterOrBubble()) {
            ...
            this.setDeltaMovement(this.getDeltaMovement().scale((double) this.getInertia()));
            if (!this.isNoGravity()) {
                this.setDeltaMovement(this.getDeltaMovement().add(0.0D, (double) (-this.getGravity()), 0.0D));
            }
            if (this.level().isClientSide) {
                if (this.lSteps > 0) {
    ...
    public Vec3 getTrailPosition(int index, float partialTick) {
        ...
        return vec31.add(vec3.subtract(vec31).scale((double) partialTick));
    }

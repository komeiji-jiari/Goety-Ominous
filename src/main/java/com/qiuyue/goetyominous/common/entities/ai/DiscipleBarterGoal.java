...
public class DiscipleBarterGoal<T extends Mob> extends Goal {
    private int progress = 100;
    public final T mob;
    private final String lootTablePath;

    public DiscipleBarterGoal(T mob, String lootTablePath) {
        this.mob = mob;
        ...
        if (this.progress <= 0) {
            ...
            if (!this.mob.level().isClientSide() && this.mob.level().getServer() != null) {
                ...
                LootTable loottable = this.mob.level().getServer().getLootData().getLootTable(
                        new net.minecraft.resources.ResourceLocation("goetyominous", lootTablePath));

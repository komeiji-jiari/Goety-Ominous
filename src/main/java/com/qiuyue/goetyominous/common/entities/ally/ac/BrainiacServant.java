...
public class BrainiacServant extends Summoned implements IAnimatedEntity {
    ...
    private static final EntityDataAccessor<Boolean> HAS_BARREL = SynchedEntityData.defineId(BrainiacServant.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Integer> TONGUE_TARGET_ID = SynchedEntityData.defineId(BrainiacServant.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> TONGUE_SHOOT_TICK = SynchedEntityData.defineId(BrainiacServant.class, EntityDataSerializers.INT);
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MOVEMENT_SPEED, AttributesConfig.BrainiacServantMovementSpeed.get())
                .add(Attributes.MAX_HEALTH, AttributesConfig.BrainiacServantHealth.get())
                .add(Attributes.FOLLOW_RANGE, AttributesConfig.BrainiacServantFollowRange.get())
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.BrainiacServantDamage.get())
    ...
    private class MeleeGoal extends Goal {
        ...
        public boolean canUse() {
            LivingEntity target = BrainiacServant.this.getTarget();
        ...
        public void tick() {
            ...
            if (target != null && target.isAlive()) {
                double dist = BrainiacServant.this.distanceTo(target);
                if (BrainiacServant.this.getAnimation() == NO_ANIMATION) {
                    if (BrainiacServant.this.getHealth() < BrainiacServant.this.getMaxHealth() * 0.5F && BrainiacServant.this.hasBarrel() && BrainiacServant.this.random.nextInt(20) == 0) {
                        BrainiacServant.this.syncAnimation(ANIMATION_DRINK_BARREL);
                    }
                    BrainiacServant.this.getNavigation().moveTo(target, 1.2D);
                    if (BrainiacServant.this.hasLineOfSight(target)) {
                        if (BrainiacServant.this.getHealth() < BrainiacServant.this.getMaxHealth() * 0.75F && dist < 20 && BrainiacServant.this.hasBarrel() && BrainiacServant.this.random.nextInt(30) == 0) {
                            BrainiacServant.this.syncAnimation(ANIMATION_THROW_BARREL);
                            BrainiacServant.this.playSound(ACSoundRegistry.BRAINIAC_THROW.get());
                        }
                        if (dist < BrainiacServant.this.getBbWidth() + target.getBbWidth() + 3.5D) {
                            BrainiacServant.this.syncAnimation(BrainiacServant.this.random.nextBoolean() ? ANIMATION_SMASH : ANIMATION_BITE);
                            BrainiacServant.this.playSound(ACSoundRegistry.BRAINIAC_ATTACK.get());
                            return;
                        }
                        if (tongueCooldown == 0 && BrainiacServant.this.random.nextInt(16) == 0 && dist < 25) {
                            BrainiacServant.this.playSound(ACSoundRegistry.BRAINIAC_LICK.get());
                            BrainiacServant.this.setLickTicks(20);
                            tongueCooldown = 15 + BrainiacServant.this.random.nextInt(15);
                        }
                    } else {
                        BrainiacServant.this.setLickTicks(0);
                    }
                }
                if (BrainiacServant.this.getAnimation() == ANIMATION_SMASH) {
                    if (BrainiacServant.this.getAnimationTick() >= 10 && BrainiacServant.this.getAnimationTick() <= 15) {
                        checkAndDealDamage(target, 2);
                    }
                }
                if (BrainiacServant.this.getAnimation() == ANIMATION_BITE) {
                    if (BrainiacServant.this.getAnimationTick() >= 10 && BrainiacServant.this.getAnimationTick() <= 15) {
    ...
    private class PickupBarrelGoal extends MoveToBlockGoal {
        ...
        public PickupBarrelGoal() {
            super(BrainiacServant.this, 1.0F, 20, 6);
        }

        protected int nextStartTick(PathfinderMob mob) {
            return reducedTickDelay(40 + BrainiacServant.this.getRandom().nextInt(40));
        }
        ...
        @Override
        public boolean canUse() {
            if (BrainiacServant.this.hasBarrel()) {
                return false;
            }
            return super.canUse();
        }
        ...
        public boolean canContinueToUse() {
            return super.canContinueToUse() && !BrainiacServant.this.hasBarrel();
        }

        public double acceptedDistance() {
            return BrainiacServant.this.getBbWidth() + 1.0F;
        }
        ...
        public void tick() {
            super.tick();
            if (blockPos != null) {
                BrainiacServant.this.lookAt(EntityAnchorArgument.Anchor.EYES, Vec3.atCenterOf(blockPos));
                if (this.isReachedTarget()) {
                    BrainiacServant.this.getNavigation().stop();
                    if (BrainiacServant.this.getAnimation() == NO_ANIMATION) {
                        BrainiacServant.this.syncAnimation(ANIMATION_BITE);
                    }
                    if (BrainiacServant.this.getAnimation() == ANIMATION_BITE && BrainiacServant.this.getAnimationTick() >= 10 && BrainiacServant.this.getAnimationTick() <= 15) {
                        if (isValidTarget(level(), blockPos)) {
                            level().destroyBlock(blockPos, false);
                            BrainiacServant.this.setHasBarrel(true);
                        }
                    }
                }

...
public class CroneRobeRenderer implements ICurioRenderer {
    ...
    private CroneRobeModel model;

    private CroneRobeModel getModel() {
        if (model == null) {
            model = new CroneRobeModel(Minecraft.getInstance().getEntityModels().bakeLayer(CroneRobeModel.LAYER_LOCATION));
        }
        return model;
    }
    ...
    public <T extends LivingEntity, M extends EntityModel<T>> void render(
        ...
        CroneRobeModel model = getModel();

        model.prepareMobModel(entity, limbSwing, limbSwingAmount, partialTicks);
        model.setupAnim(entity, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch);
        ICurioRenderer.followBodyRotations(entity, model);

        ResourceLocation texture = stack.is(ModItems.CRONE_ROBE_ALT.get()) ? TEXTURE_ALT : TEXTURE;
        VertexConsumer vertexConsumer = buffer.getBuffer(RenderType.entityCutoutNoCull(texture));
        model.renderToBuffer(poseStack, vertexConsumer, light, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
    }

    public static void register() {
        CuriosRendererRegistry.register(ModItems.CRONE_ROBE.get(), () -> new CroneRobeRenderer());
        CuriosRendererRegistry.register(ModItems.CRONE_ROBE_ALT.get(), () -> new CroneRobeRenderer());
    }
}

...
@OnlyIn(Dist.CLIENT)
public class GravityBigShulkerBulletRenderer extends EntityRenderer<GravityBigShulkerBullet> {
    ...
    private final GravityBigShulkerBulletModel<GravityBigShulkerBullet> model;

    public GravityBigShulkerBulletRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.model = new GravityBigShulkerBulletModel<>(context.bakeLayer(ModEntityLayers.GRAVITY_BIG_SHULKER_BULLET_LAYER));
    }

    @Override
    protected int getBlockLightLevel(GravityBigShulkerBullet entity, BlockPos pos) {
        return 15;
    }

    @Override
    public void render(GravityBigShulkerBullet entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation(GravityBigShulkerBullet entity) {

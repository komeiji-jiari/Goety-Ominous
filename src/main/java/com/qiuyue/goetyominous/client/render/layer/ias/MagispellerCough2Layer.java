...
@OnlyIn(Dist.CLIENT)
public class MagispellerCough2Layer<T extends LivingEntity> extends RenderLayer<T, MagispellerServantModel<T>> {
    ...
    public MagispellerCough2Layer(RenderLayerParent<T, MagispellerServantModel<T>> p_i226039_1_, EntityModelSet crap) {
        super(p_i226039_1_);
    ...
    public void render(PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entitylivingbaseIn instanceof MagispellerServant && ((MagispellerServant)entitylivingbaseIn).getGlowState() == 8 && !entitylivingbaseIn.isInvisible()) {

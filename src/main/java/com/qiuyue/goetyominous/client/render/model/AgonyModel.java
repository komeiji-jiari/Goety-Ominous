...
@OnlyIn(Dist.CLIENT)
public class AgonyModel<T extends LivingEntity> extends HierarchicalModel<T> implements ArmedModel, HeadedModel, HierarchicalArmor {
    ...
    public AgonyModel(ModelPart root) {
    ...
    public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        if (entityIn instanceof AbstractIllagerServant servant) {
            IllagerServantArmPose armPose = servant.getArmPose();
            ...
            if (entityIn.isBlocking()) {
                ...
                this.LeftArm.zRot = 0.0F;
                if (((Mob) entityIn).isAggressive() && !entityIn.getMainHandItem().isEmpty()) {
                    float f = Mth.sin(this.attackTime * (float) Math.PI);
                    float f1 = Mth.sin((1.0F - (1.0F - this.attackTime) * (1.0F - this.attackTime)) * (float) Math.PI);
                    this.RightArm.xRot = -(float) Math.PI / 2.0F - f * 1.2F + f1 * 0.4F;
            ...
            } else if (((Mob) entityIn).isAggressive()) {
                this.arms.visible = false;
                this.LeftArm.visible = true;
                this.RightArm.visible = true;
                if (entityIn.getMainHandItem().isEmpty()) {

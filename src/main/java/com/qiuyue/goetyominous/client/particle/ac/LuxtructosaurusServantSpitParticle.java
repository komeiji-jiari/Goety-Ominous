...
public class LuxtructosaurusServantSpitParticle extends TextureSheetParticle {
    private final SpriteSet sprites;
    private int onGroundTime;
    private final int luxtructosaurusId;
    private Vec3 inMouthOffset;

    protected LuxtructosaurusServantSpitParticle(ClientLevel level, double x, double y, double z, int luxtructosaurusId, SpriteSet sprites) {
        ...
        this.inMouthOffset = new Vec3(random.nextBoolean() ? 0.7F : -0.7F, -0.0F, random.nextFloat() * 1.0F - 0.25F);
        setInMouthPos(1.0F);
        this.xo = this.x;
        this.yo = this.y;
        this.zo = this.z;
    ...
    public void setInMouthPos(float partialTick) {
        if (luxtructosaurusId != -1 && level.getEntity(luxtructosaurusId) instanceof LuxtructosaurusServant entity) {
            Vec3 mouthPos = RenderLuxtructosaurusServant.getMouthPositionFor(luxtructosaurusId);
            if (mouthPos != null) {
    ...
    public static class Factory implements ParticleProvider<SimpleParticleType> {
        ...
        public Particle createParticle(SimpleParticleType typeIn, ClientLevel worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new LuxtructosaurusServantSpitParticle(worldIn, x, y, z, (int) xSpeed, spriteSet);
        }
    }
}

package com.qiuyue.someillagerservants.common.entities.ally.sar;
...
public class GrieferServant extends AbstractIllagerServant implements RangedAttackMob {
    private static final EntityDataAccessor<Boolean> KICKING = SynchedEntityData.defineId(GrieferServant.class, EntityDataSerializers.BOOLEAN);
    private static final EntityDataAccessor<Boolean> APESHIT_MODE = SynchedEntityData.defineId(GrieferServant.class, EntityDataSerializers.BOOLEAN);

    public int creeperSporeStacks;
    public int kickTicks;
    public int kickCoolDown;

    public GrieferServant(EntityType<? extends Owned> type, Level worldIn) {
        super(type, worldIn);
        this.xpReward = 0;
    }
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(2, new KickGoal(this));
        this.goalSelector.addGoal(3, new MeleePhaseGoal(this, 0.9D, true));
        this.goalSelector.addGoal(3, new AttackWithSporesGoal(this, 0.9D, 200));
        ...
        this.targetSelector.addGoal(1, (new HurtByTargetGoal(this, GrieferServant.class)).setAlertOthers());
    ...
    public static AttributeSupplier.Builder setCustomAttributes() {
        return Monster.createMonsterAttributes()
                .add(Attributes.MOVEMENT_SPEED, AttributesConfig.GrieferServantMovementSpeed.get())
                .add(Attributes.MAX_HEALTH, AttributesConfig.GrieferServantHealth.get())
                .add(Attributes.ATTACK_DAMAGE, AttributesConfig.GrieferServantDamage.get())
                .add(Attributes.FOLLOW_RANGE, AttributesConfig.GrieferServantFollowRange.get());
    }
    ...
    public ItemStack getPickedResult(HitResult target) {
        return new ItemStack(com.qiuyue.someillagerservants.common.items.sar.SarItems.GRIEFER_SERVANT_SPAWN_EGG.get());
    }
    ...
    public void performRangedAttack(LivingEntity target, float distanceFactor) {
        List<CreepieServant> nearbyCreepies = this.level().getEntitiesOfClass(CreepieServant.class, this.getBoundingBox().inflate(10.0D, 4.0D, 10.0D));
        if (nearbyCreepies.size() < 5 && this.getMainHandItem().getItem() instanceof CreeperSporesItem) {
    ...
    public static class KickGoal extends Goal {
        private final GrieferServant griefer;

        public KickGoal(GrieferServant griefer) {
    ...
    public static class MeleePhaseGoal extends MeleeAttackGoal {
        ...
        public MeleePhaseGoal(GrieferServant griefer, double speedIn, boolean useLongMemory) {
    ...
    public static class AttackWithSporesGoal extends Goal {
        ...
        public AttackWithSporesGoal(GrieferServant attacker, double moveSpeed, int maxAttackTime) {
            this(attacker, moveSpeed, maxAttackTime, maxAttackTime);
        }

        public AttackWithSporesGoal(GrieferServant attacker, double moveSpeed, int p_i1650_4_, int maxAttackTime) {
        ...
        private boolean isWalkable() {
            NodeEvaluator nodeprocessor = this.griefer.getNavigation().getNodeEvaluator();

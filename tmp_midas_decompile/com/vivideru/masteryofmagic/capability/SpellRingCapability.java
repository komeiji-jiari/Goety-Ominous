...
public class SpellRingCapability
implements ICapabilitySerializable<Tag> {
    private final ItemStack stack;
    private final LazyOptional<IItemHandler> holder = LazyOptional.of(this::getHandler);
    private SpellRingItemHandler handler;

    public SpellRingCapability(ItemStack stack) {
        this.stack = stack;
    }

    @Nonnull
    private SpellRingItemHandler getHandler() {
        if (this.handler == null) {
            this.handler = new SpellRingItemHandler(this.stack);
        }
        return this.handler;
    }
